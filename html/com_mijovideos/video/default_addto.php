...
<?php if (count($this->playlistitems)) { ?>
    <div class="mijovideos_addto_wrap">
        <div class="playlist_notification"></div>
        <h4 class="add_to_playlist_vp"><?php echo JText::_('COM_MIJOVIDEOS_ADD_TO_PLAYLIST'); ?></h4>
        <div class="mijovideos_playlist_addto_filter_vp">
            <input type="checkbox" class="mijovideos_checkbox_vp" name="add_to_top" />&nbsp;<span class="add_video_to_top_of_list_vp"><?php echo JText::_('COM_MIJOVIDEOS_ADD_TO_TOP'); ?></span>
            ...
            <?php foreach ($this->playlistitems as $item) { ?>
                ...
                <li class="mijovideos_playlist_item">
                    <a class="playlist_item" id="playlist_item<?php echo $item->id; ?>">
                        <img src="components/com_mijovideos/assets/images/tick.png" style="vertical-align: top; <?php if (!in_array($this->item->id, $playlist_videos)) { ?>visibility: hidden<?php } ?>"/>
...
<form method="post" name="adminForm" id="adminForm" action="<?php echo JRoute::_('index.php?option=com_mijovideos&task=save'); ?>">
    <div class="mijovideos_create_playlist_vp">
        <input type="text" class="mijovideos_playlist_name_vp" value="" name="playlist_title" placeholder="<?php echo JText::_('COM_MIJOVIDEOS_ENTER_PLAYLIST_NAME'); ?>" />
        <div class="mijovideos_playlist_actions_vp">
            <?php echo $this->lists['access']; ?>
            <button onclick="return false;" class="mijovideos_button_right_vp" id="create_playlist"><?php echo JText::_('COM_MIJOVIDEOS_CREATE_PLAYLIST'); ?></button>
        </div>
    </div>
...
<script type="text/javascript"><!--
    function ajaxOrder() {
        ...
        jQuery.ajax({
            url: 'index.php?option=com_mijovideos&view=video&task=ajaxOrder&video_id=<?php echo $this->item->id; ?>',
            type: 'post',
