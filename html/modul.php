 ...
 */
function modChrome_fbhtml($module, &$params, &$attribs)
{ 
	$badge          = preg_match ('/badge/', $params->get('moduleclass_sfx'))? '<span class="badge">&nbsp;</span>' : '';
	$moduleTag      = htmlspecialchars($params->get('module_tag', 'div'));
	$headerTag      = htmlspecialchars($params->get('header_tag', 'h3'));
	$headerClass    = $params->get('header_class');
	$galaxySize  = $params->get('galaxy_size');
	$moduleClass    = !empty($galaxySize) ? ' col-sm-' . (int) $galaxySize . '' : '';
	$moduleClassSfx = htmlspecialchars($params->get('moduleclass_sfx'));
	
	if (!empty ($module->content)) {
		$html = "<div class=\"fb-module\"><{$moduleTag} class=\"fb-module module{$moduleClassSfx} {$moduleClass}\" id=\"Mod{$module->id}\">" .
		...
		$html .= "<div class=\"module-content\">{$module->content}</div></div></{$moduleTag}></div>";
